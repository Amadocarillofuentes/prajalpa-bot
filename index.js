...
class LeaderboardSystem {
  constructor() {
    this.YESTERDAY_SCORES_FILE = "yesterdayScores.json";
    this.leaderboardMessages = new Map();
  }

  async initialize(client) {
    // Initialize for each guild the bot is in
    for (const guild of client.guilds.cache.values()) {
      const channelId = process.env[`LEADERBOARD_CHANNEL_${guild.id}`];
      if (channelId) {
        try {
          const channel = await guild.channels.fetch(channelId);
          const messages = await channel.messages.fetch({ limit: 1 });
          this.leaderboardMessages.set(
            guild.id, 
            messages.first() || await channel.send("Initializing leaderboard...")
          );
        } catch (error) {
          console.error(`Failed to initialize leaderboard for guild ${guild.id}:`, error);
        }
      }
    }
  }
